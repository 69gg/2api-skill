...
> 代码在 `app/upstream/base.py:AuthProvider.classify_failure`、`app/deps.py`、`app/admin.py`、`app/account.py`。
...
`[upstream]` 段可配置 `QUOTA_EXHAUSTED` 的处理方式：

```toml
[upstream]
quota_exhausted_action = "cooldown"  # "cooldown"（默认）或 "disable"
cooldown_seconds = 600.0             # 默认冷却秒数
cooldown_seconds_quota = 3600.0      # QUOTA_EXHAUSTED 覆盖值（可选）
```

- `"cooldown"`：额度耗尽后冷却指定秒数再恢复（适合每日重置的免费额度，如 Superdesign）。
- `"disable"`：额度耗尽后直接标记账号 `disabled`，不再使用（适合月额度或永不过期额度的场景）。

`CF_CHALLENGE` 仍默认走冷却；如需统一把可恢复失效都改为禁用，可在启动时调用 `app.account.set_cooldown_policy("disable")`。
...
`AuthProvider.classify_failure` 是**上游耦合点**：不同站点的 banned/quota/visitor_id/pro 模型错误关键词不同，推荐在 `app/upstream/auth.py` 覆盖 `DefaultAuthProvider.classify_failure`。若返回 `None`，则回退到 `app/deps.py:classify_failure` 的通用逻辑。详见 `references/capture-flow.md` 与 `references/upstream-adapters.md`。
